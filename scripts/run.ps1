# Start the proxy, then launch the CLI pointing at it.
...
if (-not (Test-Path $EnvFile)) {
  Write-Host "No .env found. Run 'npm run setup' first to configure your backend, then 'oscar' to launch."
  exit 1
}
...
if (Test-Path $Exe1) {
  ...
  if ($Cmd) { $CliBin = $Cmd.Source }
    ...
    Write-Error "CLI not found. Install it or run scripts/install-sdk.sh"
    exit 1
  }
}
...
try {
  ...
  $env:OSCAR_UPSTREAM_BASE_URL = "https://api.anthropic.com"
  ...
  # key so the CLI skips its OAuth login gate. In passthrough mode,
  ...
  if ($env:USE_OPENAI_API -eq "1") {
    ...
    # The CLI prefers stored OAuth credentials over the env-var key; an
    ...
    $CleanConfig = Join-Path $env:USERPROFILE ".oscar\cli-profile"
