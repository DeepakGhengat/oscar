...
test("a directory that cannot be read is skipped, not fatal", () => {
  ...
  // Use the host platform's separator: forcing "linux" here splits PATH on
  // ":", which on Windows tears `C:\Users\...` apart at the drive letter and
  // the real directory is never scanned. `cliExeNames()` includes a bare
  // "claude" on every platform, so the fixture still matches.
  const sep = process.platform === "win32" ? ";" : ":";
  const hit = findOnPath(cliExeNames(), ["\0bad", dir].join(sep));
